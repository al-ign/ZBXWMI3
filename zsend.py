...
class ZSend:
    ...
    def send(self, mydata):
        ...
        data_length = struct.pack('<I', len(mydata))
        data_to_send = b'ZBXD\x01' + data_length + b'\x00\x00\x00\x00' + mydata.encode()
        try:
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.connect((self.zserver, self.zport))
            #with open('/usr/lib/zabbix/externalscripts/global.log','a') as out:
            #    out.write(' ZSEND DATATOSEND: ' + str( data_to_send ) + '\n')
        ...
        #with open('/usr/lib/zabbix/externalscripts/global.log','a') as out:
        #        out.write(' ZSEND RESPONSE:' + str( response_header) + '\n')
        ...
        #with open('/usr/lib/zabbix/externalscripts/global.log','a') as out:
        #        out.write(' ZSEND RESPONSE RAW:' +  response_raw.decode()  + '\n')
